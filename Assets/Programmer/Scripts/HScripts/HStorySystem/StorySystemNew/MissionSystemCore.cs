...
namespace OurGame.MissionSystem
{
    ...
    public abstract class MissionRequire<T>
    {
        ...
        {
            var _handleType = GetType().GetNestedType("Handle");  //GetNestedType("Handle")：这个方法用于查找与该类型相关联的嵌套类型（内部类）。传入的字符串参数 "Handle" 是要查找的嵌套类型的名称。
            if (_handleType == null)
