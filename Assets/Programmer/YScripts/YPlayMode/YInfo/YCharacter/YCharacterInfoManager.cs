...
public static class YCharacterInfoManager 
{
    ...
    //现在是只加载了猫猫糕的信息
    public static List<YPlayerContentInfo> LoadPlayerContentInfo()
    {
        List<string> names = new List<string>();
        //创建Xml对象
        XmlDocument xmlDoc = new XmlDocument();
        ...
        XmlElement root = xmlDoc.DocumentElement;
        //现在是只加载了猫猫糕的信息
        XmlNodeList Node = root.SelectNodes("/PlayerSaveFile/CatcakeInfo/catcake");
        //初始化关卡列表
        List<YPlayerContentInfo> playerContentInfos = new List<YPlayerContentInfo>();
        foreach (XmlElement xe in Node) 
        {
            YPlayerContentInfo l = new YPlayerContentInfo();
            l.ID=xe.GetAttribute("id");
            //使用unlock属性来标识当前关卡是否解锁
            if(xe.GetAttribute("unlock")=="1"){
                l.UnLock=true;
            ...
            playerContentInfos.Add(l);
            names.Add(l.ID);
        }
        return playerContentInfos;
    }
    ...
    //通过ID获取例如猫猫糕信息 ，比如获取是否上锁，返回是否上锁，
    public static bool GetPlayerContentUnLockStatusByID(string id)
    {
        List<YPlayerContentInfo> playerContentInfos = LoadPlayerContentInfo();
        foreach (YPlayerContentInfo playerContentInfo in playerContentInfos)
        {
            if (playerContentInfo.ID == id)
            {
                return playerContentInfo.UnLock;
            }
        }
        return false;
    }
