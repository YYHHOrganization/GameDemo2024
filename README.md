...
- [ ] 最近正在学习流体模拟的效果，刚把一个正确的Compute Shader（调节亮度、饱和度、对比度）和URP的后处理联系在一起，大概的代码结构如下，其中RenderFeature里现在的Compute shader不是真正的流体模拟compute shader，而是LearnComputeShader（调节亮度那些的），③④⑤是最简单的流体模拟相关的脚本，但是还没有用render feature进行处理。

![image-20240627213229702](./assets/image-20240627213229702.png)
